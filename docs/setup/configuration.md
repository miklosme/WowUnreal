...
The checked-in Linux launcher performs this forwarding automatically:

```bash
./run_game.sh
```

It requires `WOW_DATA`, validates the core and `enUS` locale archives, and adds
the expanded `-wowdata` argument. Direct Unreal invocations still need the
