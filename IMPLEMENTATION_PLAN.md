...
- [?] Implement character rendering + equipment system from `specs/character.md` — M2 character models now render via ProceduralMeshComponent with composite skin textures (BuildCompositeTexture composites skin+face+facial_hair+underwear layers); skeleton creation with BFS topological sort handles M2's multiple root bones; 4 character types spawn in test scene (Human Male/Female, Orc Male, creature by DisplayID); armor/cloak/geoset rendering and WowEquipmentManager attachment are not yet wired; skeletal animation disabled pending UE5 runtime render data pipeline fix; screenshot verified 77.1% non-black with visible character geometry March 14, 2026
