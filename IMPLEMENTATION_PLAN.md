...
- [?] Implement the packet handler/entity system from `specs/networking.md` (`SMSG_LOGIN_VERIFY_WORLD`, `SMSG_UPDATE_OBJECT`, `SMSG_COMPRESSED_UPDATE_OBJECT`, `SMSG_DESTROY_OBJECT`, movement, chat, spells, action buttons) — verified March 14, 2026: `WowUnrealEditor` builds (0 errors), live `-autologin` run logs `AUTH_OK`, `LOGIN_VERIFY_WORLD`, `INITIAL_SPELLS` (45 spells), `ACTION_BUTTONS` (6/144 slots), `COMPRESSED_UPDATE_OBJECT` (30 entities tracked), ongoing `UPDATE_OBJECT` entity create/destroy traffic, and parsed chat messages; screenshot `networking_verify_autologin_full.png` shows terrain + sky rendering while connected (99.6% non-black); all spec acceptance criteria met
...
- [ ] Implement remaining gameplay/UI systems from `specs/overview.md` (inventory, quests, talents, social, maps)
