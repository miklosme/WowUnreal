...
- [?] Complete the typed entity hierarchy required by `specs/networking.md` — `FWowEntity` now promotes to typed player/unit/item/container/gameobject/dynamic object/corpse subclasses with typed lookup helpers and field accessors; verified March 14, 2026: `WowUnrealEditor` builds (0 errors), `WowUnreal.Entity` automation tests pass (5/5, including `TypedPromotion` and `TypedContainersAndGameObjects`), and runtime screenshot `Saved/Screenshots/typed_entity_hierarchy_verify_20260314_1450.png` was captured via UE `HighResShot` and validated 100.0% non-black.
...
- [ ] Replace the viewer fly camera with gameplay movement and chase camera from `specs/movement.md` — the production game modes still default to `AWowFlyCamera`, not `AWowPlayerCharacter`, and `ApplyLoginSpawn()` currently disables gravity and forces `MOVE_Flying`, so real terrain-following gameplay is not active in the main runtime path.
- [ ] Finish the missing movement features from `specs/movement.md` — walk/run toggle, backpedal speed handling, left/right mouse drag camera behavior, auto-run, first-person minimum zoom, swimming, flying rules, and fall damage are not implemented yet.
- [ ] Add targeting, interaction, and server-synced movement state — `CMSG_SET_SELECTION` exists, but click-to-target, NPC/object interaction, and server correction/reconciliation behavior are still missing.
...
- [ ] Complete widget mapping and anchor/layout/strata behavior in `FWowFrameManager` — `UWowUIManager::SetRootCanvas()` is never called, parent/child widget hierarchy is not realized, and parsed `<Texture>` / `<FontString>` layers are not turned into rendered UMG widgets.
- [ ] Replace the remaining hardcoded Lua API stubs with real implementations (`Source/WowUI/Private/LuaApi/LuaStubs.cpp`) — `UnitName`, `UnitClass`, `UnitRace`, screen size, action bar, spell/item, addon, binding, CVar, and targeting APIs still return dummy values or no-op.
...
- [ ] Add the frame methods and WoW UI API surface needed by FrameXML/addons — most frame mutators plus `Texture` / `FontString` behavior in `LuaFrameApi.cpp` are still stubbed no-ops, so FrameXML scripts cannot drive real layout, visibility, text, or textures.
...
- [ ] Complete FrameXML / addon boot sequencing from `specs/ui-framexml.md` — `LoadFrameXml()` and addon XML loading do not recursively resolve `<Include>` directives, fonts are only logged instead of loaded, `VARIABLES_LOADED` / `PLAYER_ENTERING_WORLD` are not fired, and `ADDON_LOADED` is not emitted per addon with arguments.
- [ ] Implement `OnUpdate`, mouse, drag, and gameplay-driven UI events from `specs/ui-framexml.md` — the event system only invokes `OnEvent`, and `OpcodeToEvent()` is currently unused, so live quest/chat/action bar/unit updates do not propagate into FrameXML.
...
- [ ] Complete the remaining `specs/sky-atmosphere.md` acceptance criteria — zone-blended sky colors/fog work, but visible sun/moon discs and a deterministic time-of-day override for dawn/noon/dusk/night verification are still missing.
...
- [ ] Restore UE screenshot API-only verification — `WowWorldManager::SaveViewportPng()` currently shells out to macOS `screencapture`, which violates the project screenshot rules and bypasses `FScreenshotRequest`.
...
- [ ] Implement character rendering + equipment system from `specs/character.md` — `WowCharacterTexture::BuildCompositeTexture()` still returns the base skin only, armor/cloak/geoset rendering is absent, and `WowEquipmentManager` is not wired into spawned characters.
- [ ] Spawn player/NPC character models from live entity data in the main world — character rendering currently only exists in the isolated character test scene and is not connected to networked entity creation.
- [ ] Implement audio system from `specs/audio.md` — `AWowAudioManager` can load music, but the main world never calls `SetCurrentZone()`, ambience/day-night switching are not implemented, and combat/volume handling remain incomplete.
